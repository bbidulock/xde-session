...
Peform proxy services also for L<wmaker(1)> dock applications.  This is
seldom necessary as L<xde-dock(1)> provides a proxy for session
management of L<wmaker(1)> dock applications on its own.
...
C<WM_STATE> property to these windows in compliance with ICCCM 2.0 for
...
they are treated as non-dockapp windows).  Dock applications typically
also have the C<WM_CLASS>, C<WM_NAME>, C<WM_COMMAND>,
C<WM_CLIENT_MACHINE> and C<WM_CLIENT_LEADER> properties set.
...
window managers; cannot identify WindowMaker dock applications; cannot
...
L<xdg-launch(1)>, L<smproxy(1)>, L<xsm(1)>.
