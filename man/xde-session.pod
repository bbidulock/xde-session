...
user logs in.  Alternatively, B<xde-session> will allows the user to
...
F<$HOME/.config/xde-sessions/$display/sessions/> directory, where
...
B<xde-session> supports the X11R6 XSMP Session Management Protocol.
...
The X11R6 XSMP protocol provides the following features (that are not
