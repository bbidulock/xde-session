...
B<xde-traymgr> is a C<C> language application program that manages
system tray icons.  It provides startup, ordering and X11 session
management for system tray icons.
...
Assume normal operation.  Launch system tray icons and monitor the
system tray.
...
Ask a running instance to launch the editor.  The editor is used to
edit the system tray icons that will be launched at startup and the
order of system tray icons in the tray.
...
When B<xde-traymgr> first starts, it establishes a connection with the
...
When B<xde-traymgr> starts, it also monitors for startup notification so
that it may identify applications that are invoked that establish a
system tray icon.
...
The B<xde-traymgr> selects for events on the manager window so that it
...
"icon" window identified in the message.  When received, B<xde-traymgr>
will monitor for XEMBED events associated with the "icon" window.

After establishing input selection with the manager window,
B<xde-traymgr> will start launching system tray icon applications as
...
B<xde-traymgr> had no known bugs at the time of release.  Please report
