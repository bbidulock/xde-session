...
B<xde-chooser> can be run from an XDM to launch an X session, run as a
desktop application to switch a running X session, or run from the
command line to test its operation.  These three uses are detailed
below.
...
There are, in fact, no standards specifications for C<XSession> files.
...
which C<XSession> F<.desktop> files are shipped, are as follows:
L<2bwm(1)>,
L<adwm(1)>,
L<aewm(1)>,
L<aewm++(1)>,
...
The C<XSession> files read by B<xde-chooser> are capable of
...
manager in various ways.  An example C<XSession> file is as follows:

=head3 [Desktop Entry] Section

An example C<[Desktop Entry]> section from the F<adwm.desktop>
C<XSession> file is as follows:
...
The Desktop Entry specification version in conformance with the XDG
L<Desktop Entry Specification>.  Should always be C<1.0>, and defaults
to C<1.0> if this entry is not present.

=item C<Encoding=UTF-8>

The encoding of this file in conformance with the XDG
L<Desktop Entry Specification>.  Should always be C<UTF-8>, and defaults
to C<UTF-8> if this entry is not present.

=item C<Type=XSession>

Provides the type of desktop entry file.  The only valid types in the
formal Desktop Entry specification are C<Application> and C<Autostart>;
however, C<XSession> is typically used for X sessions and the type will
default to C<XSession> if this field is not provided.  This field may be
set to C<Application> when the file is to also reside with other
applications.
...
=item C<GenericName=Window Manager>

=item C<GenericName[fr]=Gestionnaire de Fenêtres>

=item C<GenericName[pt]=Gestor de Janelas>

The generic name of the desktop entry in accordance with specification.
The generic name should be the same for all window managers and
therefore for all C<XSession> files.

=item C<Comment=An advanced dynamic window manager for X>
...
=item C<Terminal=false>

Optional per specification: false when not provided.

=item C<NoDisplay=true>

Should be set to true.  This field is ignored by B<xde-chooser>, but
will be observed by XDG desktop entry programs such as menus and
launchers if the file is to also reside within an applications
directory.

=item C<Hidden=true>

Should be included an set to C<true> when the desktop entry is simply
used to occlude another C<XSession> entry in a lower priority XDG
directory.  B<xde-chooser> uses normal XDG search rules, so it is common
to set C<$XDG_CONFIG_DIRS=/etc/xdg/xde:/etc/xdg> and
C<$XDG_DATA_DIRS=/usr/local/share/xde:/usr/share/xde:/usr/local/share:/usr/share>
so that the shipped C<XSession> files are discovered first and the
undesirable ones that appear in F</usr/share/xsessions> can be occluded
by a file of the same name in F</usr/share/xde/xsessions> with
the C<Hidden> flag set to C<true>.

=item C<Categories=WindowManager;Application;System;>

Categories for window managers should always be set as above.

=item C<StartupNotify=true>

Specifies whether the window manager will perform desktop startup
notification completion in accordance with the
L<Desktop Startup Notification Specification>.  L<adwm(1)> does, so in
this case it is set to C<true>.  It is assumed that if a window manager
performs desktop startup notification for itself and indicates that it
supports C<_NET_STARTUP_ID>, C<_NET_STARTUP_INFO_BEGIN> and
C<_NET_STARTUP_INFO> in C<_NET_SUUPPORTED> that it also supports
assisting non-startup-notification-aware clients.

=item C<X-GNOME-Autostart-Notify=true>

=item C<X-GNOME-Autostart-Phase=WindowManager>

=item C<X-GNOME-Provides=windowmanager>

=item C<X-GNOME-WMName=ADWM>

=item C<X-AppInstall-Package=adwm>

=item C<X-AppInstall-Popcon=0.00>

=item C<X-AppInstall-Section=aur>

=item C<X-Arch-Package=adwm 0.7.4-1>

=item C<X-Arch-Package-Repository=aur>

=item C<X-Arch-Package-Name=adwm>

=item C<X-Arch-Package-Version=0.7.4-1>

=item C<X-Arch-Package-Description=Advanced dynamic window manager for X>

=item C<X-Arch-Package-Groups=None>

=item C<X-Arch-Package-URL=http://github.com/bbidulock/adwm>

=item C<X-Arch-Package-Votes=3>

=item C<DesktopNames=ADWM>

=item C<X-LightDM-DesktopName=adwm>

=item C<Actions=XDE;>

=back

Note that the C<OnlyShowIn> and C<NotShowIn> fields are pointless (as
the current desktop has not yet been set), and they should not appear in
the C<XSession> file.

=head3 [Window Manager] Section
...
=item C<SessionManaged=false>

=item C<StartupNotification=true>

=item C<X-XDE-Managed=true>

=back

=head3 [Desktop Action ACTION] Section
...
=item C<[Desktop Action XDE]>

=item C<Name=XDE>

=item C<Comment=adwm in an XDE session>

=item C<Icon=xde>

=item C<TryExec=/usr/bin/xde-session>

=item C<Exec=xde-session -desktop ADWM -startwm start-adwm>

=item C<DesktopNames=XDE:ADWM>

=item C<X-LightDM-DesktopName=xde>
