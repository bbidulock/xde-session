...
B<xde-xsession> [I<OPTIONS>] [I<SESSION>]

=head1 DESCRIPTION

B<xde-xsession> is a C<C>-language program that can be lauched from, or
...
session names or the special names: C<logout>, C<default>, C<current> or
C<choose>.
...
Do not choose a session, but simply run (typically without prompting)
whichever session the user last ran.  (The default is located in the
file F<$XDG_CONFIG_HOME/xde/current>).  When no current session is
configured, or when the last run session is not usable (e.g. missing
executable), this argument has the same effect as C<choose>.
...
entry files.  Will be overriden by the B<--desktop> command line option.
...
lookup algorithm.  Will be overriden by the B<--file> command line
...
Specifies whether or not to perform tool wait on autostarted tasks where
startup completion notification can be deteremined from the
...
unspecified is to display a splash window.  Will be overriden by the
...
=item B<SessionManagement>=I<BOOLEAN>
...
=item B<SessionManagementProxy>=I<BOOLEAN>
...
=item B<ScreenLocking>=I<BOOLEAN>
...
=item B<ScreenLockerProgram>=I<STRING>
...
locking instead of internal screen locker when B<ScreenLocking> is C<true>.
...
=item B<ThemeManagement>=I<BOOLEAN>

Sepcifies whether or not to perform desktop theme management for the
...
applications.  The default is C<true>.  Ignored when B<ThemeManagement>
...
icons.  The default is C<true>.  Ignored when B<ThemeManagement> is
