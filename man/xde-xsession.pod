...
F<autostart> subdirectory of each use and system XDG config directory.
...
=item B<X-GNOME-Autostart-Phase>={B<Applications>|B<Desktop>|B<Initialization>|B<Panel>|B<WindowManager>}

=item B<X-GNOME-Provides>=I<ARRAY>;{B<filemanager>|B<login-sound>|B<panel>|B<windowmanager>}
...
B<SessionManagementProxy> is C<true>.  Cannot be overridden with a
