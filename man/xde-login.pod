...
B<xde-login> is a simple C<C> language program the establishes a user
X session.
...
and arguments with the PAM environment.
...
The options that are detailed below are recognized by B<xdg-login>.
...
When set, the B<XDG_SESSION_DESKTOP> environment variable will be set to
