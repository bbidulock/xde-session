...
B<xde-dockmgr> is a C<C> language application program that manages dock
...
relies on the appearance of a dock (or window manager support a dock)
...
B<xde-dockmgr> expects to be launched before the window manager.  It can
be launched by XDG autostart entry, but when launched in this fashion it
must be launched in the C<Intializing> phase of autostart.  That is, to
be effective, it must be launched before the window manager and before
the first dock application is launched.

B<xde-dockmgr> assumes a session manager that provides total ordering of
clients on restoration.  That is the only way of ensuring that dock
...
B<xde-dockmgr> does not harmfully interact with session manager proxies
...
allows it to perform the same functions as B<xde-dockmgr>.

If dock applications are launched with XDG startup notification, or
startup notification assistance (see L<xdg-launch(1)>), B<xde-dockmgr>
...
Note that dock applications do not normally perform X11R6 session
management, they do often perform the older B<WM_SAVEYOURSELF> approach
and so need a session management proxy.  Rather than trying to figure
out session management properties in real time, B<xde-dockmgr> requests
a save-yourself phase 2 and determines the necessary properties at that
point.  It is not a normal proxy in the sense that it performs
connection by connection session management, it is a proxy in the sense
that it registers as a client on behalf of dock applications and
coordinates their restart when they it restarts.
...
save state.  If a dock application does not support he
...
B<xde-dockmgr> tracks the full life cycle of each dock application.
...
B<xde-dockmgr>.  That is not the case with system tray applications.
...
B<xde-dockmgr> sets the B<SM_CLIENT_ID> property on the client leader
...
When the session managers sends a save yourself, B<xde-dockmgr> requests
a save-yourself phase 2.  When phase 2 begins, B<xde-dockmgr> examines
the state and position of all dock applications and stores their primary
ICCCM 2.0 properties as well as any present WMH/WinWM, EWMH/NetWM window
manager hints.
...
The following environment variables are examined by B<xde-dockmgr>:
