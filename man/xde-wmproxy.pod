...
Management.  It peforms this action as a meta-WM in parallel or in lieue
...
It can perform setup and autostart capabilities for window maangers as
well.  Note that L<xde-autostart(1)> can perform autostart capabilities
too; however, B<xde-wmproxy(1)> can make autostarted tasks children of
...
outside of the first by invoking it using xde-wmproxy as a wrapper to
