...
Application.desktop -- Application .desktop file format

=head1 SYNOPSIS

B<$XDG_DATA_HOME>I</applications/*.desktop>

B<$XDG_DATA_DIRS>I</applications/*.desktop>


=head1 DESCRIPTION

This manual page describes the format of the C<Application> F<.desktop> file format.

=head1 FILE FORMAT

There is only one format for the F<.desktop> file of type C<Application>.
...
The optional generic name of the entry (e.g. B<Web Browser>).
...
When set to I<false> in the application file, or when not specified and
the B<StartupWMClass> key is not set, should the launcher send a I<new:>
startup notification message, it should also send a I<remove:> message
once it has completed launching the application.  Should it send a
I<new:> message in such cases, the I<new:> message should contain the
I<SILENT=1> key.

When this key is not present and the B<StartupWMClass> key is present,
the application launcher should await the mapping of a window with the
appropriate class or name before sending the I<remove:> message.  Any
sent I<new:> message should not contain the I<SILENT> parameter.

When the window manager lists support for atoms
B<_NET_STARTUP_INFO_BEGIN> and B<_NET_STARTUP_INFO>, the window manager
will be responsible for sending the I<remove:> message once a window of
the necessary I<res_class> or I<res_name> has mapped.
...
L<XSession.desktop(5)>,
L<Autostart.desktop(5)>,
L<Application.desktop(5)>,
