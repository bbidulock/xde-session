...
xde-greeter - log into an X session

=head1 SYNOPSIS

B</usr/lib/X11/xdm/xde-greeter> [I<OPTIONS>]

=head1 DESCRIPTION

B<xde-greeter> is an L<xdm(8)> greeter replacement that provides a much
nicer appearing login screen for L<xdm(8)>.  It is designed to let
L<xdm(8)> do the heavy lifting of coordinating the majority of the XDMCP
protocol while providing a more aesthetic appearance.

=head1 OPTIONS

No options are necessary.  When launched by L<xdm(8)>, it will provide
the appropriate arguments.  B<xde-greeter> can be launched directly on
the command line of an x-terminal to test its appearance and operation.
In this mode no options are required.  See L</USAGE> for more
information.
...
When no command option is specified, B<xde-greeter> executes the program.
...
B<xde-greeter> provides a set of general options that are not supported
...
greeter options.  So that an intermediate script is not necessary,
...
greeter.  As a result, the use of the following command line options are
...
shell script between L<xdm(8)> and B<xde-greeter> that adds the
options:

 #!/bin/sh
 /usr/lib/X11/xdm/xde-greeter -prompt "Welcome to X!" \
...
For example:
 ...
 #!/bin/sh
 /usr/lib/X11/xdm/xde-greeter -banner /usr/lib/X11/xdm/xde/banner.png $@
...
example:
 ...
 #!/bin/sh
 /usr/lib/X11/xdm/xde-greeter -splash /usr/lib/X11/xdm/xde/splash.jpg $@
...
Most values that affect the look and feel of B<xde-greeter> may also be
...
resource name and class (C<xde-greeter>, C<XDE-Greeter>).  When no
resource is found with the specific name and class, L<xdm(8)> resources
with name and class (C<xlogin.Login>, C<Xlogin.Login>) will also be
examined for specified resources.
...
F</usr/share/X11/app-defaults/Chooser> file.  Likewise, B<xde-greeter>
specific resource defaults are contained in the
F</usr/share/X11/app-defaults/XDE-Greeter> file, in accordance with the
convention of naming applications default files after their resource
class. See also L</FILES>.

=head2 XDE GREETER RESOURCES

B<xde-greeter> observes the following X11 Resource Manager resources when
loaded by L<xdm(8)> prior to execution:

=over 4

=item B<xde-greeter>.B<x>: I<DOUBLE>
...
=item B<xde-greeter>.B<y>: I<DOUBLE>
...
=item B<xde-greeter>.B<debug>: I<INTEGER>

Provides the debugging level in the same fashion as the B<-debug>
command line option.

=item B<xde-greeter>.B<banner>: I<PATH>
...
=item B<xde-greeter>.B<side>: {B<left>|B<right>|B<top>|B<bottom>}
...
=item B<xde-greeter>.B<user.default>: I<USERNAME>
...
=item B<xde-greeter>.B<autologin>: {B<true>|B<false>}

When I<true>, specifies that the user defined in
B<xde-greeter>.B<user.default> should be automatically logged into a
...
=item B<xde-greeter>.B<vendor>: I<VENDOR-ID>
...
=item B<xde-greeter>.B<prefix>: I<MENU-PREFIX>
...
=item B<xde-greeter>.B<login>.B<permit>: {B<true>|B<false>}
...
=item B<xde-greeter>.B<login>.B<remote>: {B<true>|B<false>}
...
=item B<xde-greeter>.B<xsession.chooser>: {B<true>|B<false>}

Specifies the value of the B<--xsessions> option.

=item B<xde-greeter>.B<xsession.execute>: {B<true>|B<false>}

=item B<xde-greeter>.B<xsession.default>: I<APPID>

Specifies the value of the B<--default> option.

=item B<xde-greeter>.B<setbg>: {B<true>|B<false>}

Specifies the value of the B<--nosetbg> option.

=item B<xde-greeter>.B<transparent>: {B<true>|B<false>}

Specifies the value of the B<-transparent> option.

=item B<xde-greeter>.B<face>: I<XFTFONT>
...
=item B<xde-greeter>.B<greetFace>
...
=item B<xde-greeter>.B<greetColor>
...
=item B<xde-greeter>.B<promptFace>
...
=item B<xde-greeter>.B<promptColor>
...
=item B<xde-greeter>.B<inputFace>
...
=item B<xde-greeter>.B<inputColor>
...
=item B<xde-greeter>.B<failColor>
...
=head2 XDM GREETER RESOURCES

When specific B<xde-greeter> resources are not specified,
B<xde-greeter> falls back the using the resources specified for its
...
Xft support.
This resource not used by B<xde-greeter>.
...
window.  This may be use to change the appearance of the screen around
...
for a file user here is F<Xstartup>.  See L<xdm(8)/Startup Program>.
...
F<Xsession>.  See L<xdm(8)/Session Program>.
...
F<Xreset>.  See L<xdm(8)>/Reset Program>.
...
reset scripts to the value of this resource.  The default for this
resource is F</etc:/bin:/usr/bin:/usr/sbin:/usr/ucb>.  Note the absence
of C<.> from this entry.  This is a good practice to follow for root; it
...
generats and uses authorization for local server connections.  If
...
=item B<DisplayManager>.I<DISPLAY>.B<userAuthDir>: I<STRING>

When L<xdm(8)> is unable to write the usual user authorization file
(C<$HOME/.Xauthority>), it creates a unique file name in this directory
and points the environment variable C<XAUTHORITY> at the created file.
It uses F</tmp> by default.

=item B<DisplayManager>.I<DISPLAY>.B<chooser>: I<STRING>

Specifies the program run to offer the host menu for Indirect queries
redirected to the special host name C<CHOOSER>.
F</usr/lib/X11/xdm/chooser> is the default.
See L<xdm(8)/XDMCP Access Control> and L<xdm(8)/Chooser>.

=item B<DisplayManager>.I<DISPLAY>.B<greeter>: I<STRING>

Specifies the program run to offer a graphical login prompt.  When this
resource is specified, the value of B<DisplayManager>.B<greeterLib> is
ignored for the specified I<DISPLAY>.

=item B<DisplayManager>.I<DISPLAY>.B<xlogin>: I<STRING>

Specifies the program run to offer a graphical login prompt and launch
user sessions for local displays.  When this resource is specified, the
value of B<DisplayManager>.B<greeterLib> is ignored for the specified
I<DISPLAY>.
...
=item F</usr/lib/X11/xdm/xde-greeter>

The default location of the B<xde-greeter> executable.
...
The default location of the background splash to display on the login
...
B<XDE-Greeter> resources are contained in the F</etc/X11/xdm/xde>
subdirectory.  Application defaults are not loaded from anywhere else
for security.

=item F</usr/share/X11/app-defaults/XDE-Greeter>

X resource defaults for B<xde-greeter>.
...
B<xde-greeter> had no known bugs at the time of release.  Please report
