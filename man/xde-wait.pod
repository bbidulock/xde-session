...
Wait for all desktop resources (window manager, system tray, desktop
pager and composite manager) before running the command or exiting.
...
start a start a window manager and sleep for two seconds before starting
...
B<xde-wait> had no know bugs at the time of release.  Please report bugs
in current versions to
L<https://github.com/bbidulock/xde-ctools/issues/>.
...
Please see C<xde-wait --copying> for copying permissions.
