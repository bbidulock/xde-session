...
specified is C<XDE>.  The desktop evironment must be specified when
...
Invokes B<lxsession(1)> compatability mode, and specifies the session
...
manager appears, but before executing any other commands or autostarting
...
optionaly prefixede with a C<@> to indicate that the process should be
restarted when it exits abnormally.  Tasks executed from this file will
be started before any specified by the B<-x> option, or autostarted from
XDG autostart desktop files.

In L<lxsession(1)> compatability mode, this option will default to the
...
sense of the B<-a> flag is for compatability with L<lxsession(1)>.
...
When autostarting tasks for which startup notification completion can be
...
options), the program will wait I<DELAY> millseconds before starting the
next task.  I<GUARD> defaults to 200 milliseconds.  I<DELAY> defaults to
zero (0) milliseconds.  The default is to perform toolwait.  To disable
toolwait, specify C<--toolwait 0:0>.
...
Speciy the output language for XDG desktop file derived information.
...
options is recognized for compatablity with other tools.
...
monitoring autostarted tasks in the background.  This option causes the
...
Increment or set the debug level to I<LEVEL>.  The default debuggin
