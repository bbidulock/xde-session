...
B<xde-xlogin> is being used as a greeter program.
...
greeter.  L<xdm(1)> does not provide this option, but it can easily be
...
The default location of the B<xde-xlogin> executable.  As this is not
useful for normal users, it does not reside in the F</usr/bin>
directory.
...
Because this greeter is associated with XDE, a complete set of
