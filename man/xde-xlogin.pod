...
B<xde-xlogin> is an L<xdm(8)> xlogin and chooser replacement that
provides a much nicer appearing login screen for L<xdm(8)>.
...
indicates to B<xde-xlogin> that it was not launched by L<xdm(8)> or an
...
B<xde-xlogin> is being used as a I<chooser> program.  Note that none
...
shell script between L<xdm(8)> and B<xde-xlogin> that adds the
options:
...
B<xde-xlogin> provides for both L<gdm(1)> style and L<dtsession(1)>
style session management by optionally providing the user with the
ability to select an X session from the available sessions as well as a
saved session from the following list of saved sessions:

=over

=item I<current>

Starts the user's most recent session.  The most recent session is the
last saved current session.

=item I<home>

Starts the user's home session.  The user's home session is the session
that was last saved as a home session.

=item I<display-current>

Creates a display-specific session and starts the first of the following
...
Creates a new display-specific session and starts the user's I<home>
session if it exits; otherwise, starts a new user (initial) session.

=item I<failsafe>

Starts a fail-safe session.

=back

Although the user's list of saved sessions is not known until after the
user logs in, the dialog presents all of the session choices.  If a user
selects a session that does not exist, B<xde-xlogin> takes the following
...
B<xde-xlogin> starts a new user session.

=item I<current>

B<xde-xlogin> starts the user's home session, if it exists.  If it does
not, it starts a new user session.
...
B<xde-xlogin> posts a warning dialog stating that a new session will be
created.  The warning dialog contains two actions:
...
If a I<display-home> session was selected, B<xde-xlogin> creates a new
display-specific session and starts the user's generic I<home> session
if it exists.  If it does not exist, it starts a new user session.

If a I<display-current> session was selected, B<xde-xlogin> creates a
new display-specific session and starts a I<display-home>, I<home>, or
new user session, depending on which it finds first.

=back

=back

Note that B<xde-xlogin> does not actually launch these sessions, but
passes the selection to the session manager chosen.

A new user (initial) session, follows the rules for launching an X
session.
...
=item B<xde-xlogin>.B<theme.sound>: I<NAME>
...
Xft support.
This resource is not used by B<xde-xlogin>.
