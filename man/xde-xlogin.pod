...
chooser options listed above under L</XDM OPTIONS>.  So that an
intermediate script is not necessary, L<xdm(1)> provides an
F<XResources> files containing X resources that are used instead of
command line options to tailor the look and feel of the greeter and
...
greeter and chooser.  L<xdm(1)> does not provide this option, but it can easily be
...
When unspecified, the program searches for an image file named
F<${XDG_VENDOR_ID:--}splash.{xpm,png,jpg,svg}> in the subdirectory
F<images> in XDG data directories.
