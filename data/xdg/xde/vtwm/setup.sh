...
vers=${2:-${XDE_WM_VERSION:-$(LANG= $prog -V 2>/dev/null|awk '/VTWM/{print$NF;exit}')}}
[ -n "$vers" ] || vers="5.5.0"
...
if [ -s "$here/version" ]; then
	...
	if [ "$generate" = "true" ]; then
		...
		if which xde-menugen >/dev/null 2>&1; then
			xde-menugen --format="$name" --desktop="$desktop" --launch --output="$priv/menu" &
		elif which xdg-menugen >/dev/null 2>&1; then
			xdg-menugen --format="$name" --desktop="$desktop" --launch --output="$priv/menu" &
		fi
	fi
fi
...
if [ ! -f "$priv/rc" -o "$priv/rc.m4" -nt "$priv/rc" ]; then
	...
	export VTWM_CONFIG_RDIR="$rdir"
	export VTWM_CONFIG_XDIR="$xdir"
